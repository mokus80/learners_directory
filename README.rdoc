== About
The Rails Learners Directory is a crowdsourced directory of resources to learn Ruby on Rails. It enables learners to find resources appropriate for them. The Learners Directory is for learners and coaches of all levels who are looking for user-recommended learning resources. It is also for resource-writers and aggregators to better spread their work.  
...
The Learners Directory was created by Laura Wadden and Nicole Felhösi during the 2013 Rails Girls Summer of Code (http://www.railsgirlssummerofcode.org). Many thanks to Duana Stanley, Deepak Jois, Tobias Pfeifer, Oliver Hookins and Daniel Kehoe for their coaching and support.  

== Features
* User sign-in and profile page
...
We don't want to hardcode the environment variables, so we use the foreman gem to set them. Create a new file called '.env'. It'll ask if you're sure, click yes. By putting the '.' at the front of a filename, the file isn't visible in the Finder. It's a private file so no one else is able to see it. 
...
When we use the foreman gem on the command line, we access this file and it sets the environment variables using the .env file and starts the server for us.
...
Create a user with a password. Remember these because you'll need them later. I'll use 'username' for the username, 'password' for the password and 'mydatabase' for the database.  
...
Allows this user to create a database.
...
The Rails Learners Directory was built by Nicole Fehlösi and Laura Wadden (@laurawadden) during the 2013 Rails Girls Summer of Code as part of Team Rails Grrls. Fehlösi and Wadden were coached by Duana Stanley, Deepak Jois, Oliver Hookins, Tobias Pfeiffer and Daniel Kehoe. To contact the owners, please message through github or tweet to @RailsGrrls.